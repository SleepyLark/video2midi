...
class GLSpinBox():
    ...
    def draw(self):
        ...
        if self.edit_mode:
            # Focused = blue border
            glColor4f(0.2, 0.2, 0.35, 0.95)
        else:
            glColor4f(0.2, 0.2, 0.2, 0.9)

        DrawQuad(self.x, self.y, self.x + self.w, self.y + self.h)

        # Border color
        if self.edit_mode:
            glColor4f(0.3, 0.6, 1.0, 1.0)  # blue highlight
        else:
            glColor4f(1, 1, 1, 1)

        DrawRect(self.x, self.y, self.x + self.w, self.y + self.h)

        # Buttons
        glColor4f(0.3, 0.3, 0.3, 1)
        DrawQuad(self.x + self.w - self.btn_width, self.y,
                 self.x + self.w, self.y + self.h/2)
        DrawQuad(self.x + self.w - self.btn_width, self.y + self.h/2,
                 self.x + self.w, self.y + self.h)

        DrawRect(self.x + self.w - self.btn_width,
                 self.y, self.x + self.w, self.y + self.h)

        # Arrows
        glColor4f(1, 1, 1, 1)
        DrawTriangle(self.x + self.w - (self.btn_width/2 + 4), self.y + self.btn_width/2-2,self.btn_width/2-2, 1)
        DrawTriangle(self.x + self.w - (self.btn_width/2 + 4), self.y + self.btn_width-2,self.btn_width/2-2, 0)

        # Value text
        text = self.edit_text if self.edit_mode else str(self.value)
        drawText((self.x + 4, self.y + self.h, 1), (255,255,255), text)

        # Label
        if self.label:
            drawText((self.x, self.y, 1),
                     (255,255,255), self.label)
    ...
    def update_mouse_down(self, mpx, mpy, btn):
        ...
        # Click on main box → enter edit mode
        if (self.x < mpx < self.x + self.w - self.btn_width) and \
           (self.y < mpy < self.y + self.h):
            self.edit_mode = True
            self.edit_text = str(self.value)
            return

        # If focus is lost after typing 
        if self.edit_mode is True:
            self.commit_edit()

        # Otherwise leave edit mode
        self.edit_mode = False

        # Buttons
        if mpx > self.x + self.w - self.btn_width:
            # Up
            if self.y < mpy < self.y + self.h/2:
                self.set_and_callback(self.value + self.step)
            # Down
            elif self.y + self.h/2 < mpy < self.y + self.h:
                self.set_and_callback(self.value - self.step)

    # ------------------------------------------------------
    # Helper to clamp + call callback
    # ------------------------------------------------------
    def set_and_callback(self, v):
        v = max(self.vmin, min(self.vmax, v))
        self.value = v
        self.edit_text = str(v)

        if self.update_func:
            self.update_func(self, v)

    # ------------------------------------------------------
    # Keyboard input (pygame key constants)
    # ------------------------------------------------------
